...
1. Does `manual.md` list type combinations not covered by the TypeRule/IntrinsicTypeRule registry?
...
4. Are there COSY edge cases in `manual.md` not handled?
...
- Registry has fewer than 4 type combinations for a numeric operator -- check `manual.md` Appendix A for missing types
...
```json
{
  "concerns": [
    {
      "severity": "error",
      "argument": "COS registry is missing DA type support -- manual.md shows COS(DA)->DA is required",
