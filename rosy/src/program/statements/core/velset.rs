...
use crate::{
    ast::*,
    program::expressions::{core::variable_identifier::VariableIdentifier, Expr},
    transpile::{
...
impl FromRule for VelsetStatement {
    fn from_rule(pair: pest::iterators::Pair<Rule>) -> Result<Option<Self>> {
        ...
        let vector_pair = inner.next()
            .context("Missing vector variable in VELSET!")?;
        ...
        Ok(Some(VelsetStatement { vector_ident, component_expr, value_expr }))
    }
}
...
impl Transpile for VelsetStatement {
    fn transpile(&self, context: &mut TranspilationInputContext) -> Result<TranspilationOutput, Vec<Error>> {
        ...
        let vector_output = self.vector_ident.transpile(context)
            .map_err(|e| add_context_to_all(e, "...while transpiling vector identifier in VELSET".to_string()))?;
