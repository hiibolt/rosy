...
impl FromRule for SinhExpr {
    fn from_rule(pair: pest::iterators::Pair<Rule>) -> Result<Option<Self>> {
        anyhow::ensure!(pair.as_rule() == Rule::sinh_fn, "Expected sinh_fn rule, got {:?}", pair.as_rule());
...
impl Transpile for SinhExpr {
    fn transpile(
        ...
        let serialization = format!("RosySINH::rosy_sinh({})?", inner_output.as_ref());
